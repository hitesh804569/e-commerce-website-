...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!isset($_POST[CSRF_TOKEN_NAME]) || $_POST[CSRF_TOKEN_NAME] !== $_SESSION[CSRF_TOKEN_NAME]) {
        ...
        // Validate input (trim email to avoid trailing/leading spaces)
        ...
        if (empty($errors)) {
            try {
                ...
                if ($user) {
                    ...
                    // Support legacy plaintext passwords by auto-migrating to hashed
                    if (!$verified) {
                        $info = password_get_info($stored);
                        if ($info['algo'] === 0 && hash_equals($stored, $password)) {
                    ...
                    if ($verified) {
                        ...
                        if ($user['role'] === 'admin') {
                            ...
                            $redirect = isset($_SESSION['redirect_after_login']) ?
                                $_SESSION['redirect_after_login'] : BASE_URL . '/index.php';
                            unset($_SESSION['redirect_after_login']);
                            header('Location: ' . $redirect);
                        }
                        exit;
...
<body class="bg-gray-100 min-h-screen flex items-center justify-center">
    <div class="bg-white p-8 rounded-lg shadow-md w-full max-w-md">
        <h1 class="text-2xl font-bold mb-6 text-center">Login to YBT Digital</h1>
